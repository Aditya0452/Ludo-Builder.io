...
export default function GameOffline() {
  const [players] = useState<Player[]>([
    { id: 1, name: 'Player 1', color: 'red', pieces: [0, 0, 0, 0], isActive: true },
    { id: 2, name: 'Player 2', color: 'green', pieces: [0, 0, 0, 0], isActive: false },
    { id: 3, name: 'Player 3', color: 'yellow', pieces: [0, 0, 0, 0], isActive: false },
    { id: 4, name: 'Player 4', color: 'blue', pieces: [0, 0, 0, 0], isActive: false },
  ]);
  ...
  const getPlayerColorClass = (color: string) => {
    switch (color) {
      case 'red': return 'bg-ludo-red border-red-600';
      case 'green': return 'bg-ludo-green border-green-600';
      case 'yellow': return 'bg-ludo-yellow border-yellow-600';
      case 'blue': return 'bg-ludo-blue border-blue-600';
        ...
        <div className="grid lg:grid-cols-4 gap-6">
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-center space-x-3">
                    <div className={`w-6 h-6 rounded-full border-2 ${getPlayerColorClass(player.color)}`} />
                    <div className="flex-1">
                      <h4 className="text-white font-medium">{player.name}</h4>
                      <p className="text-sm text-purple-200 capitalize">{player.color}</p>
                    </div>
                    {index === currentPlayer && (
          ...
          <div className="lg:col-span-2">
            <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-white text-center">Ludo Board</CardTitle>
              </CardHeader>
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <Card className="bg-black/20 border-white/10 backdrop-blur-sm">
              ...
              <CardContent className="text-center space-y-4">
                <div className={`w-16 h-16 rounded-full mx-auto border-4 ${getPlayerColorClass(players[currentPlayer].color)}`} />
                <p className="text-white font-medium">{players[currentPlayer].name}</p>
                <p className="text-purple-200 text-sm capitalize">{players[currentPlayer].color} Player</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-black/20 border-white/10 backdrop-blur-sm">
              ...
              <CardContent className="text-center space-y-4">
                ...
                <p className="text-purple-200 text-sm">
                  {diceValue === 6 ? 'Roll again! You got a 6!' : `You rolled: ${diceValue}`}
                </p>
              </CardContent>
            </Card>
          </div>
